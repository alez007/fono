...
async fn exercise_tts(cfg: &TtsCfg, secrets: &Secrets) -> Result<usize> {
    let tts = fono_tts::build_tts(cfg, secrets, &[], std::path::Path::new(""))?
...
async fn exercise_groq_e2e(secrets: &Secrets) -> Result<()> {
    ...
    // Was `en-conversational.wav`; that fixture was removed because
    ...
    let tts = fono_tts::build_tts(&tts_cfg, secrets, &[], std::path::Path::new(""))
